...
function slugifyYear(label: string, fallback: number) {
  return (
    label
      .toLowerCase()
      .replace(/[^\w]+/g, '-')
...
async function readReferenceHtml(relativePath: string) {
...
function toRemoteUrl(src: string) {
  try {
...
async function copyAssetToHistory(
  src: string,
  yearSlug: string,
  subfolder = ''
) {
  ...
  try {
    ...
    if (!remoteUrl) throw new Error('Invalid URL');
    await downloadToFile(remoteUrl, destinationPath);
    const relativeFolder = subfolder ? `/${subfolder}` : '';
    return `/history/${yearSlug}${relativeFolder}/${fileName}`;
  } catch (error) {
    missingSources.add(`Image not found for year ${yearSlug}: ${src} (${(error as Error).message})`);
    return null;
  }
}

async function copyImageAsset(src: string, yearSlug: string) {
  return copyAssetToHistory(src, yearSlug);
}

async function copyDownloadAsset(src: string, yearSlug: string) {
...
async function ensureReferenceFiles() {
  ...
  if (!(await pathExists(historyPath))) {
    try {
      ...
      missingSources.add(`Failed to download history.html: ${(error as Error).message}`);
    }
  }
  ...
  const { load } = cheerio as { load: (html: string) => any };
  const $ = load(localHistory);
  const yearLinks = new Set<string>();
  $('#wsite-content a').each((_: unknown, el: unknown) => {
  ...
  for (const href of yearLinks) {
    ...
    try {
      ...
      missingSources.add(`Failed to download ${url}: ${(error as Error).message}`);
    }
  }
}

async function convertHtmlToMdx(html: string, yearLabel: string, yearSlug: string) {
  const cheerio = await ensureCheerio();
  if (!cheerio) {
    return `> History content for ${yearLabel} is unavailable because required parsing tools are not installed.`;
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const cheerioModule = cheerio as { load: (html: string) => any };
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const $: any = cheerioModule.load(html);
  ...
  async function rewriteHref(href: string) {
    ...
    if (isTeam) {
      if (pathPart.startsWith('/history')) return '/about/history';
  ...
  const children = $(node)
      .contents()
      .toArray()
      .map((child: unknown) => serialize(child))
      .filter(Boolean)
      .join(' ');
  ...
  body.children().each((_: unknown, elem: unknown) => {
...
async function writeFileSafe(target: string, contents: string) {
...
async function importHistory() {
  ...
  let introBody =
    'Team Voltage history content is not yet available in this workspace. Add local reference files and re-run the import.';
  const introTitle = 'Team Voltage History';
  const introDescription =
    'Year-by-year program milestones from the founding of Team Voltage through the latest FIRST Robotics Competition games.';
  ...
  const yearMetas: HistorySection[] = [];
  ...
  await writeFileSafe(
    path.join(yearsOutputDir, 'index.json'),
